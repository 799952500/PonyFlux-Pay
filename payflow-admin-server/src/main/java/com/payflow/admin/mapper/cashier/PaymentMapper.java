...
public interface PaymentMapper extends BaseMapper<Payment> {
     ...
     * 按订单号查询支付记录
     */
    @Select("SELECT * FROM cashier_payments WHERE order_id = #{orderId} ORDER BY created_at DESC")
    List<Payment> findByOrderId(@Param("orderId") String orderId);

    /**
     * 按渠道查询支付记录
     */
    @Select("SELECT * FROM cashier_payments WHERE pay_channel = #{payChannel} ORDER BY created_at DESC")
