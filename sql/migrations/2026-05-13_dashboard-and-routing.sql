...
-- ============================================================
-- 商业智能仪表盘 + 阶梯费率 + 智能路由 — 数据库迁移
-- Date: 2026-05-13
-- Feature: 004-business-optimization
-- ============================================================
...
-- 2.1 渠道表新增手续费率
ALTER TABLE channels ADD COLUMN IF NOT EXISTS fee_rate DECIMAL(6,4) DEFAULT NULL COMMENT '渠道默认手续费率（如0.0060=0.6%）';

-- 2.2 商户表新增费率计算模式和商户组
ALTER TABLE merchants ADD COLUMN IF NOT EXISTS rate_calc_mode VARCHAR(16) DEFAULT 'flat' COMMENT '费率计算模式: flat=全额匹配, segmented=分段累计';
ALTER TABLE merchants ADD COLUMN IF NOT EXISTS merchant_group VARCHAR(64) DEFAULT NULL COMMENT '商户所属费率组';

-- 2.3 收银台渠道表新增手续费率（用于智能路由）
ALTER TABLE cashier_channels ADD COLUMN IF NOT EXISTS fee_rate DECIMAL(6,4) DEFAULT NULL COMMENT '渠道手续费率（如0.0060=0.6%）';
