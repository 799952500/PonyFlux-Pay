    ...
    <main class="flex-1 flex flex-col items-center px-5 pb-10 pt-4">
      <div class="w-full max-w-[480px]">
        ...
        <div v-else-if="receipt" class="receipt-card">
          <h1 class="text-xl font-bold text-center pf-text-title mb-1">{{ t('receipt.title') }}</h1>
          <p class="text-center pf-text-muted text-xs mb-6">{{ t('receipt.subtitle') }}</p>

          <dl class="space-y-3 text-sm">
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.receiptNo') }}</dt>
              <dd class="text-right font-mono pf-text-title break-all">{{ receipt.receiptNo ?? '—' }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.generatedAt') }}</dt>
              <dd class="text-right pf-text-body">{{ receipt.generatedAt ?? '—' }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.merchant') }}</dt>
              <dd class="text-right pf-text-body">{{ receipt.merchantName }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.product') }}</dt>
              <dd class="text-right pf-text-body">{{ receipt.subject }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.orderNo') }}</dt>
              <dd class="text-right font-mono text-xs pf-text-body break-all">{{ receipt.orderId }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.amount') }}</dt>
              <dd class="text-right">
            ...
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.amountWords') }}</dt>
              <dd class="text-right pf-text-body text-xs leading-snug max-w-[240px]">{{ receipt.amountCn }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.channel') }}</dt>
              <dd class="text-right pf-text-body">{{ receipt.payChannel }}</dd>
            </div>
            <div class="receipt-row">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.paidAt') }}</dt>
              <dd class="text-right pf-text-body">{{ receipt.payTime || '—' }}</dd>
            </div>
            <div class="receipt-row receipt-row--last">
              <dt class="pf-text-muted shrink-0">{{ t('receipt.transactionNo') }}</dt>
              <dd class="text-right font-mono text-xs pf-text-body break-all">{{ receipt.transactionNo || '—' }}</dd>
            </div>
          </dl>

          <button type="button" class="pay-btn mt-6" @click="downloadPdf">{{ t('receipt.downloadPdf') }}</button>
        </div>

        <p v-else class="text-center pf-text-muted py-16 text-sm">{{ t('receipt.empty') }}</p>
      </div>
    </main>
  </div>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    if (data.displayLanguage) {
      await applyDisplayLocale(data.displayLanguage, { persist: false })
    }
  } catch {
    receipt.value = null
  } finally {
