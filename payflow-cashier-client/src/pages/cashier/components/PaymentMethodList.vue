<template>
  <div class="order-card p-5 mb-4">
    <p class="text-[#0F172A] font-semibold text-sm mb-4 flex items-center gap-2">
    ...
    <div class="space-y-2.5">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-[#0F172A] font-medium text-sm">{{ method.methodName }}</p>
            <p v-if="method.discount" class="text-emerald-600 text-xs mt-0.5 font-medium">
              🎉 {{ method.discount.name }}
            </p>
            <p v-else class="text-[#94A3B8] text-xs mt-0.5">安全、快捷</p>
          </div>
        </div>
