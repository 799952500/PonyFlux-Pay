    ...
    <div class="content-card">
      ...
      <el-table table-layout="auto" v-loading="loading" :data="list" stripe size="small" class="data-table">
        <el-table-column label="对账状态" prop="reconStatus" width="110" align="center">
          <template #default="{ row }">
            <el-tag size="small" :type="tagTypeOf(RECON_STATUS_TAG, row.reconStatus, 'warning')">
              {{ labelOf(RECON_STATUS_LABEL, row.reconStatus) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="处理状态" prop="handleStatus" width="100">
          <template #default="{ row }">
            <span v-if="row.handleStatus">{{ labelOf(RECON_HANDLE_LABEL, row.handleStatus) }}</span>
            <span v-else class="cell-empty">—</span>
          </template>
        </el-table-column>
        <el-table-column label="对账渠道/账户" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.reconChannel">{{ channelLabel(row.reconChannel) }} / {{ row.accountCode }}</span>
            <span v-else>—</span>
          </template>
        </el-table-column>
